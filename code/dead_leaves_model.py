...
def compute_dead_leaves_model(img_size=500, sigma=3, shape='disk', num_iter=5000, rmin=0.01, rmax=1, n_color=10):
    ...
    ref_img = cv2.imread('../dataset/grass1.jpg')  # choose your reference image to color
    ref_img = cv2.cvtColor(ref_img, cv2.COLOR_BGR2RGB)
    model, _ = segmentation(ref_img, n_color=n_color)
...
if __name__ == '__main__':
    cluster_index, model = compute_dead_leaves_model(img_size=512, sigma=5, shape='disk')
    ...
    plt.imshow(result)
    plt.show()
    imsave('../results/dead_leaves/generated_grass1.png', result)
